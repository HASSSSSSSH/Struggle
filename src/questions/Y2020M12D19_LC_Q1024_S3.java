 ...
 * 空间复杂度: O(T)
 */
public class Y2020M12D19_LC_Q1024_S3 {

    public static void main(String args[]) {
        Y2020M12D19_LC_Q1024_S3 instance = new Y2020M12D19_LC_Q1024_S3();
    ...
    public int videoStitching(int[][] clips, int T) {
        int[][] dp = new int[T + 1][clips.length];

        for (int t = 1; t <= T; t++) {
            for (int i = 0; i < clips.length; i++) {
                dp[t][i] = clips.length + 1;
            }
        }
        // if (clips[0][0] == 0) {
        //     for (int t = 1; t <= clips[0][1]; t++) {
        //         dp[t][0] = 1;
        //     }
        // }
            ...
            for (int t = 1; t <= T; t++) {
                ...
                if (t > clipStart && t <= clipEnd) {
                    if (i == 0) {
                        ...
                        dp[t][i] = Math.min(dp[t][i], dp[clipStart][i - 1] + 1);
                    }
                }
            }
        }

        return dp[T][clips.length - 1] <= clips.length ? dp[T][clips.length - 1] : -1;
    }
}
